...
class ApiSignService
{
    ...
    public function sign(array $data, string $key)
    {
        ...
        if (!isset($data['app-id']) || !isset($data['timestamp']) || !isset($data['nonce'])) {
            throw new ApiSignException("签名错误", ApiSignException::PARAMS_ERROR);
        }

        //应用数据
        $app_sign = $this->driver->getInfo($data['app-id']);
        if (!$app_sign) {
...
//        echo 'sign:'.$str;
    ...
    public function check(array $data, string $key)
    {
        ...
        if ($this->config['timeout'] && $this->config['timeout'] > 0 && ($time + $this->config['timeout'] < $ts || $time - $this->config['timeout'] > $ts)) {
        ...
        if($this->config['replay']){
            ...
            $key = 'nonce:' . $ip . ':' .$time. $noncestr;

            if ( Redis::exists($key)) {
                throw new ApiSignException("请求失效，重复请求", ApiSignException::REQUEST_INVALID);
            }else{
                //存储 noncestr

                Redis::setex($key, $this->config['replay_timeout'],true); // 设置过期时间，这里假设 replay_timeout 是一个合适的过期时间
            }
        }
    }
}
